...
# --- CONFIGURATION ---
# Resolves to: project_root/data/silver/crypto_binance
...
def check_continuity(coin_id: str, threshold_minutes: int = 2) -> None:
    ...
    if not file_path.exists():
        print(f"⚠️  {coin_id.upper()}: No Silver Parquet file found.")
        return
    ...
    try:
        ...
        print(f"❌ {coin_id.upper()}: Failed to read Parquet file. Error: {error}")
        return
    ...
    # Reporting Logic
    if gaps.empty:
        print(f"✅ {coin_id.upper()}: Perfect Continuity ({len(df):,} rows)")
    else:
        max_gap = time_diffs.max()
        # Retrieve the timestamp of the first gap found
        first_gap_date = gaps["source_updated_at"].iloc[0]

        print(f"⚠️  {coin_id.upper()}: Data Gaps Detected!")
        print(f"   - Total Gaps: {len(gaps)}")
        print(f"   - Max Gap Duration: {max_gap}")
        print(f"   - First Gap Occurred: {first_gap_date}")

def main() -> None:
    """
    Orchestrates the continuity check across all available Silver assets.
    """
    print("🕵️  Initiating Gap Hunt (Threshold: >2 minutes).")
    print(f"📂 Scanning Directory: {SILVER_DIR}\n")
    ...
    if not coin_folders:
        print("⚠️  No asset folders found. Have you run the Silver Pipeline?")
        return
